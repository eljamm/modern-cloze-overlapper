...
export function makeClozeDeletions(str, separator, numPrompts = 1) {
  ...
  function clozeRange(i) {
    ...
    const rangeEnd = Math.max(
      Math.min(i + 3, separators.length + 3 - numPrompts),
      2,
    );
  ...
  separators.forEach((m, i) => {
    ...
    const text =
      hintStart > -1 ? textWithHint.substring(0, hintStart) : textWithHint;
    const hint =
      hintStart > -1 ? `::${textWithHint.substring(hintStart + 2)}` : "";
...
function updateGeneratedCloze(e) {
  if (
    /** @type {HTMLButtonElement} */ (e.submitter).name !== "generateButton"
  ) {
    return;
  }
  e.preventDefault();
  const { separator, numPrompts, clozeInput, generatedCloze } =
    /** @type {ClozeGeneratorForm} */ (this).elements;
  generatedCloze.value = makeClozeDeletions(
    clozeInput.value,
    new RegExp(separator.value, "ug"),
    numPrompts.valueAsNumber,
  );
}
  ...
  class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      const dialog = /** @type {HTMLDialogElement} */ (
        this.getElementsByTagName("dialog")[0]
      );
      /** @type {HTMLButtonElement} */ (
        dialog.nextElementSibling
      ).addEventListener("click", () => dialog.showModal(), { passive: true });

      const form = /** @type {ClozeGeneratorForm} */ (
      ...
      form.elements["numPrompts"].addEventListener("input", reportValidity, {
        passive: true,
      });
...
class ClozeNode extends NodeArray {
  ...
  render(config, cardInfo) {
    ...
    const renderClozeSpan = (className, html) =>
      this.#makeClozeSpan(className, html ?? super.render(config, cardInfo))
        .outerHTML;
    ...
    if (
      this.isContext ||
      any(this.parents, (p) => p.isContext) ||
      this.childCardNums.has(cardInfo.cardNum) ||
      (cardInfo.isBack &&
        any(this.parents, (p) => p.cardNum === cardInfo.cardNum))
    ) {
...
export function parseCloze(clozeSrc, config, cardInfo) {
  ...
  let lastIndex = (NEXT_TOKEN_RE.lastIndex = 0),
    m;
  while ((m = NEXT_TOKEN_RE.exec(clozeSrc))) {
    ...
    if (m[0].startsWith("{{c")) {
      const clozeBefore =
        nestedClozes.findLast((c) => c.lastCloze)?.lastCloze ??
        topLevel.lastCloze;
      const c = new ClozeNode(
        parseInt(m[0].substring(3, m[0].length - 2), 10),
        nestedClozes.at(-1)?.parentsAndSelf,
        clozeBefore,
      );
      nestedClozes.push(c);
  ...
  for (const c of currentCardClozes) {
    ...
    for (
      let b = c.clozeBefore, i = infIfNeg(config.contextBefore);
      b && i > 0;
      --i
    ) {
      b.isContext = true;
    ...
    for (
      let a = c.clozeAfter, i = infIfNeg(config.contextAfter);
      a && i > 0;
      --i
    ) {
      a.isContext = true;
...
export function stripHtmlFromMathJax(html, cardInfo) {
  ...
  return html.replaceAll(MATHJAX_RE, (mathjax) => {
    ...
    ["script", "style"].forEach((t) =>
      Array.prototype.forEach.call(dom.getElementsByTagName(t), (e) =>
        e.remove(),
      ),
    );
...
function defineMathJaxClozeCommands(clozeContainer) {
  ...
  const rgbStartIdx = color.startsWith("rgb(")
    ? 4
  ...
  const rgb =
    rgbStartIdx !== -1
      ? color
          .substring(rgbStartIdx, color.length - 1)
          .split(", ")
          .map((s) => Math.round(+s))
...
function parseConfig(elementId) {
  const config = /** @type {string} */ (
    /** @type {HTMLTemplateElement} */ (document.getElementById(elementId))
      .content.textContent
...
export function renderClozes(partialConf, onRender) {
  ...
  const renderedCloze = stripHtmlFromMathJax(
    parseCloze(clozeSource.innerHTML, config, cardInfo).render(
      config,
      cardInfo,
    ),
    cardInfo,
    ...
    new Promise((resolve) => {
      requestAnimationFrame(() => {
        if (revealAllButton && cardInfo.hasUnrevealedClozes) {
          revealAllButton.addEventListener("click", revealAllCallback, {
            once: true,
            passive: true,
          });
